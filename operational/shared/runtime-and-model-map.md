...
The audit's output is a cleaned doc plus, where relevant, a note for `agent-definition-principles.md` (so the Claude-ism doesn't get re-authored next time) and `dev/patches/claude-code/` if the fix belongs at the base-prompt patch layer (H40).
...
- **Claude base-prompt patch (H40)** — captured as intention; ties to `dev/patches/claude-code/`. Codex levels have no Claude base prompt, so the patch is Claude-runtime-only by construction.
