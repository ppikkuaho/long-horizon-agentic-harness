...
- The work is version-sensitive and belongs in a version-keyed patch registry for the Claude Code harness. Do not hand-edit; register the patch.
...
There is a living, external design-principles reference for LLM and agent design. It informs this system, but its principles are **held softly here** (L46): the *mechanisms* transfer, the specific *prescriptions* re-express.
...
- **CC base-prompt patch (H40, intent)** → coding-assistant default must stop fighting role framing; via the Claude Code harness patch registry; Opus seats only.
