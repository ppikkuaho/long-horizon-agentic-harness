...
Total: 17 passed, 0 failed, 0 errors under `python3`.
...
- The shell has no bare `python` executable, so the exact requested command cannot run here. The same unittest suite passes under the available `python3` interpreter.
...
- Best-effort bus nudge was not delivered. The bus control commands failed in this environment, and the target participant was not reachable.
