...
On L1 **final-accept**, the **daemon / control plane promotes the finished product OUT of the gitignored `/runtime/` project node TO the delivery destination captured at intake** (§3). This is the load-bearing new mechanism. After delivery, **project teardown / reclaim** of the `/runtime/` tree is a **deferred follow-on** (register D7), not part of v1 promotion.
