...
The audit layer is **not** a passive metrics-reader bolted onto the side. It is a **parallel-important** function: a self-improvement capability with a god-view over the whole system and its own development methodology for proposing interventions. It is the second consumer above, but its role is large enough to design as a layer in its own right. (The full design for optimizer-L1, when it is built, will live in a dedicated document; for now the Internal Affairs workspace is its holding location.)
