...
- The **role-and-contract bundle** the agent reads at boot — the per-level `operational/L{n}/{soul,role,config,…}.md`, the always-loaded shared contract docs `operational/shared/{comms-protocol,agent-lifecycle,agent-definition-principles,runtime-and-model-map,git-protocol}.md`, the shared minimal prompt `operational/shared/system-prompt.md`, and the **read-only design docs** (`design/*.md`).
...
- **Reads** of the **role-and-contract bundle the agent READS at boot** — these are now READ documents the agent pulls in place at their harness-root paths, NOT the `--system-prompt-file` (the `--system-prompt-file` is the constant shared `operational/shared/system-prompt.md`, which the CC process itself reads at boot, H40 Decision B):
  - the shared minimal prompt `operational/shared/system-prompt.md` (read by the CC process at boot — must be read-allowed);
  - the per-level docs `operational/L{n}/{soul,role,config,…}.md` (+ level extras: L1 handbook/intake, L3 planning-template, L5 swe-handbook);
  - the always-loaded shared contract docs `operational/shared/{comms-protocol,agent-lifecycle,agent-definition-principles,runtime-and-model-map,git-protocol}.md` (each scoped to its own level set per its header — comms-protocol/agent-lifecycle/runtime-and-model-map = all levels, agent-definition-principles = L1–L4, git-protocol = code-levels; see `ROLE-RESOLUTION.md` §2). Read-allowing all five is harmless — a level may *read* more than it *loads*; only the load-scope is per-doc.
  These are delivered as documents (the node-local spawn brief's "Identity — Load These Documents" load-manifest points at them), READ IN PLACE, never inline-flattened (DAEMON §3.2 binding, §7).
- **Reads** of the read-only harness **design docs** under `design/*.md` (brief.md is pointer-not-payload; upstream intent is pulled on demand, WORKSPACE-SCHEMA line 217). These bundle docs are all read-only, under the harness root — which is exactly why containment is **write-jail-not-read-jail**: refs resolve in place without flattening.
...
- **Reads otherwise OPEN** — the boot bundle the agent READS in place (the shared minimal prompt `operational/shared/system-prompt.md` that the CC process reads at boot, the per-level `operational/L{n}/{soul,role,config,…}.md`, the always-loaded `operational/shared/{comms-protocol,agent-lifecycle,agent-definition-principles,runtime-and-model-map,git-protocol}.md`), design docs (`design/*.md`), the rest of `.cc-pinned/config`, the pinned binary, `/etc`, system libs all readable so role resolution and read-only harness docs work (VERIFIED: denied-secret read failed *while* allowed-workspace + role-doc reads succeeded under the same profile). The cross-project read-deny is the complement that closes the §5 exfil surface to the agent's own readable set.
...
- **System prompt:** `--system-prompt-file operational/shared/system-prompt.md` — the ONE shared minimal prompt, IDENTICAL for L1–L5, constant across every spawn (NOT a per-level file, NOT `role.md`; promoted 2026-06-05 per H40 Decision B). This replaces base block 2, keeps the 57-char identity line, and keeps the **default 24-tool SWE set** incl. search/web/Bash/Edit/Write. **VERIFIED interactive + OAuth-compatible by H40.**
- **Role:** delivered as **documents the agent READS at boot** (per-level `operational/L{n}/{soul,role,config,…}.md` + the always-loaded shared contract docs), pointed at by the node-local spawn brief's load-manifest — never baked into the prompt. The per-binding `role_variant` (DAEMON §3.2) selects WHICH bundle the chokepoint assembles into the brief; the `--system-prompt-file` above stays constant.
...
- **Permission posture:** OAuth + `--system-prompt-file operational/shared/system-prompt.md` (the constant shared minimal prompt; the role is READ as docs per the spawn brief's load-manifest, not baked into the prompt) + skip-permissions + full tools incl. search, journaled as a named `containment_posture` record. `--bare` rejected.
