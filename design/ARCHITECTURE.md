...
**Transport is the bus; truth is the docs (F33).** The old filesystem-inbox model is superseded. Real-time coordination flows over the **bus**; durable truth lives in **docs** in the work nodes. A message is a **pointer or a nudge** ("report.md updated", "blocked — see node X"), not the payload. Because the truth is in the docs, message delivery is **best-effort** — a dropped nudge costs a little latency, not correctness, since any level can re-read the durable artifact. This decouples transport reliability from system correctness.
...
- **Personal/life domains** -- Different function, different structure. Personal-life domains operate differently than project work.
...
**Decision (2026-03-25):** A separate personal-assistant system and the L1-L5 agent architecture are confirmed as separate systems — not integrating. The personal-assistant system is a single-agent assistant with persistent memory (capture, consolidation, domain-based knowledge). L1-L5 is a multi-agent delegation hierarchy for project work. They solve different problems with different architectures: the personal-assistant system makes one agent better through memory; L1-L5 makes work better through delegation. They may coexist in the same repository but do not share state, infrastructure, or runtime.
